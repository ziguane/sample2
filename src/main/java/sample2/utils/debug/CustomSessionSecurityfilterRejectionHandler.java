...
public class CustomSessionSecurityfilterRejectionHandler extends SessionSecurityfilterRejectionHandler {
	 ...
	 * Constructor.
	 *
	 * @param securitySessionConfiguration Security Session Configuration session
	 *                                     store
	 */
//    public SessionSecurityfilterRejectionHandler(SecuritySessionConfiguration securitySessionConfiguration) {
	...
	/**
	 * If request is HTML then do a 303 Redirect, if not respond with correspondent
	 * HTTP Status code.
	 * 
	 * @param request   {@link HttpRequest} being processed
	 * @param forbidden if true indicates that although the user was authenticated
	 *                  he did not had the necessary access priviledges.
